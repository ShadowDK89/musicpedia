...
const References: React.FC = () => {
  ...
  useEffect(() => {
    console.log(referenceStoreData.sources == undefined ? true : false);
  }, []);
  ...
  function applyRating() {
    if (currentUser) {
      setError("");
      const docRef = db.collection("references").doc(referenceStoreData.id);
      ...
      return db.runTransaction((transaction) => {
        return transaction
          ...
          .then((doc) => {
            ...
            let newRefs = {
              refs: referenceStoreData.sources,
            };
            newRefs.refs[refNumControl - 1].rating.push(userInputRating);

            let refs = newRefs.refs;
            transaction.update(docRef, { refs: newRefs });
            dispatch(setReferencesData({ id: referenceStoreData.id, refs }));

            /*             let newRefs = doc.data()?.refs;
            newRefs.push(newReference);
  ...
  async function handleSubmit(newReference: TReference) {
    let getRefNum = referenceStoreData.sources.length + 1;
    newReference.referenceNumber = getRefNum;
    console.log(getRefNum);
  ...
  function displayRatingHtml(ref: TReference) {
    return (
      <React.Fragment>
        <input
          data-id={ref.referenceNumber}
          type="radio"
          id={`${ref.referenceNumber}-star1`}
          name="star"
          value="5"
        ...
        <input
          data-id={ref.referenceNumber}
          type="radio"
          id={`${ref.referenceNumber}-star4`}
          name="star"
          value="2"
          onClick={getUserRating}
        />
        <label
          htmlFor={`${ref.referenceNumber}-star4`}
          title="Not Good"
        ></label>
        <input
          data-id={ref.referenceNumber}
          type="radio"
          id={`${ref.referenceNumber}-star5`}
          name="star"
          value="1"
          onClick={getUserRating}
        />
  ...
  const rateMeHtml = (
    <div className="rate-me">
      <h5>Is this ok?</h5>
      <a href="#" onClick={applyRating}>
        Yes
      </a>
      <a href="#" onClick={cancelRating}>
        No
      </a>
    </div>
  );
  let referenceItemsHtml: JSX.Element[] = [];

  referenceItemsHtml = referenceStoreData.sources.map((ref) => {
    return (
      <li key={ref.referenceNumber}>
        ...
        <div className="ref-rating">
          <div className="rate-stars">{displayRatingHtml(ref)}</div>
      ...
      <section className="references musician-page">
        <div className="ref-container">
          <div className="ref-heading list-container">
            ...
            <p>
              Help evaluate if the reference is correct. Together we make this
              site reliable.
            </p>
          ...
          <div className="ref-list-container">
            <ul className="ul-layout">{referenceItemsHtml}</ul>
          </div>
        </div>
      </section>
    );
